...
export const HTML_CONTENT = `<!DOCTYPE html><html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
<footer id="footer" style="width:100%;text-align:center;padding:24px 0 12px 0; background:rgba(0,0,0,0.04);margin-top:40px;">
  <div style="margin-bottom:8px;">
    <a href="https://notes.suouaki.me/admin" target="_blank" title="notes" class="footer-icon">
      <img src="https://cdn.jsdelivr.net/gh/simple-icons/simple-icons/icons/notes.svg" alt="notes" style="width:28px;height:28px;margin:0 10px;vertical-align:middle;">
    </a>
...
export default {
    ...
    async fetch(request, env) {
        ...
        if (url.pathname === '/api/getNotes') {
            ...
            const row = await db.prepare('SELECT notes FROM notes WHERE userId=?').bind(userId).first();
            return new Response(row && row.notes ? row.notes : JSON.stringify([]), { status: 200 });
        }
        // 保存笔记（D1实现）
        if (url.pathname === '/api/saveNotes' && request.method === 'POST') {
            const { userId, notes } = await request.json();
            await db.prepare('INSERT OR REPLACE INTO notes(userId, notes) VALUES (?, ?)').bind(userId, JSON.stringify(notes)).run();
        ...
        if (url.pathname === '/api/getStyles') {
            ...
            const row = await db.prepare('SELECT styles FROM styles WHERE userId=? AND type=?').bind(userId, type).first();
            return new Response(row && row.styles ? row.styles : JSON.stringify({}), { status: 200 });
        }
        // 保存标题样式（D1实现）
        if (url.pathname === '/api/saveStyles' && request.method === 'POST') {
            const { userId, type, styles } = await request.json();
            await db.prepare('INSERT OR REPLACE INTO styles(userId, type, styles) VALUES (?, ?, ?)').bind(userId, type, JSON.stringify(styles)).run();
            return new Response(JSON.stringify({ success: true }), { status: 200 });
        }
        // 处理未找到的路径
