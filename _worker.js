...
.sidebar:not(.expanded) .sidebar-close {
...
@media (max-width: 1080px) {
    ...
    /* 手机端卡片布局调整 */
    .note-container {
        grid-template-columns: repeat(2, 1fr); /* 手机端每行 2 个卡片 */
        gap: 10px;
    }
    .note {
        width: 100%; /* 卡片占满网格单元 */
...
.note {
    position: relative;
    background-color:rgb(78, 63, 200, 0.5);
    padding: 15px;
    border-radius: 10px;
    box-shadow: 0 4px 8px rgba(214, 241, 173, 0.93);
    transition: transform 0.2s ease, box-shadow 0.2s ease;
    width: auto; /* 宽度根据内容自适应 */
    min-width: 800px; /* 最小宽度，避免过窄 */
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-close" title="收起目录">✕</div>
...
function bindSidebarToggleEvents() {
    ...
    if (!sidebar) return;
    const sidebarIcon = sidebar.querySelector('.sidebar-icon');
